...
public class PowerEstimator implements Runnable {
    ...
    //JFL: patch update time
    public static final int ITERATION_INTERVAL = 200; // 1 second
    ...
    /* Indefinitely collect data on each of the power components. */
      ...
      /* Sleep until the next iteration completes. */
            ...
            HashMap<Integer, Integer> uidPowerJFL = new HashMap<Integer, Integer>();
            HashMap<Integer, String> uidNameJFL = new HashMap<Integer, String>();

            int totalPower = 0;
            // components: LCD, CPU, WIFI, 3G, AUdio....
            for (int i = 0; i < components; i++) {
           ...
           */
                ...
                for (int j = 0; j < uidPower.size(); j++) {
                    ...
                    Integer puid = uidPowerJFL.get(uid);

                    String name = sysInfo.getUidName(uid, pm);

                    //if (power > 0)
                    //  System.out.println(name + "=" + power);


                    //if (!name.contains("EnergyCollector") && !name.contains("PowerTutor")) // EXCLUDING ENERGYCOLLECTOR
                    {
                        //if (power > 0)
                        //	  System.out.println(name + "=" + power);
                        //if (name.contains("Stegano"))
                        //	  Log.w("JFL", "STEGANO: "+ name);
                        if (puid == null) {
                            uidPowerJFL.put(uid, Integer.valueOf(power));
                            uidNameJFL.put(uid, name);
                        } else {
                            uidPowerJFL.put(uid, Integer.valueOf(power + puid.intValue()));
                            String oldName = uidNameJFL.get(uid);
                            if (!oldName.equals(name)) {
                                Log.e("JFL", "****** ERROR ******** This uid " + uid + " change its name from" +
                                        uidNameJFL.get(uid) + " to " + name);
                            }
                        }
                    }
...
//          if (name.contains("Stegano"))
//          {
//        	  Intent infoSS = new Intent("jf.andro.SteganoSender");
//        	  if (name.contains("Stegano: Sender"))
//        		  infoSS.putExtra("sender", power);
//        	  if (name.contains("Stegano: Receiver"))
//        		  infoSS.putExtra("receiver", power);
//        	  context.sendBroadcast(infoSS);
//          }
                    ...
                    // Adding to history the power data of this UID
            ...
            //JFL: HERE WE CAN PATCH FOR LOGGING POWER
            String uidMessage = "";
            for (Integer uid : uidPowerJFL.keySet()) {
                uidMessage += uid + "=" + uidPowerJFL.get(uid) + "=" + uidNameJFL.get(uid) + ";";
            }
            //System.out.println(uidMessage);
            Intent infoSS = new Intent("jf.andro.SteganoSender");
            infoSS.putExtra("allUID", uidMessage);
            context.sendBroadcast(infoSS);
